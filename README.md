...
- **전투 중 유닛 커맨드 메뉴** (`もどる`/`終了`/`情報`/`通信`/`かばう`/`隊長`/`環境`, `解除`/`防御`/`回復` 등): 디스크 전체(압축 해제 포함)를 뒤져봐도 텍스트로는 안 잡혀서, **그래픽(버튼 이미지)으로 그려져 있을 가능성이 높습니다.** 후보로 보이는 `BPV1` 포맷(`SYSDATA/SAVESLG.CBD` 등) 압축 해제까지는 성공했지만, 스프라이트 아틀라스 내부 구조(버튼 위치/크기 표, 픽셀 포맷)는 아직 완전히 못 풀었습니다. 이미지라면 텍스트 치환이 아니라 그래픽을 다시 그려 넣어야 해서 별도의 훨씬 큰 작업이 필요합니다.
