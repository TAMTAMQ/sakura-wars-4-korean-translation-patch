...
> ⚠️ **완역 패치가 아닙니다.** 대부분의 대화 스크립트는 번역되어 있지만, 아직 못 찾은 대화 스크립트나 **그래픽(이미지)으로 그려져 있는 일본어**(예: 전투 커맨드 메뉴 버튼 등)는 번역되어 있지 않습니다.
>
...
- **전투 중 유닛 커맨드 메뉴** (`もどる`/`終了`/`情報`/`通信`/`かばう`/`隊長`/`環境`, `解除`/`防御`/`回復` 등)를 비롯한 **그래픽(이미지)으로 그려진 일본어**는 디스크 패치(`sw4.dcp`)로는 바뀌지 않지만, 위 "2-1. 텍스처(그래픽) 한글패치"의 Flycast Custom Textures 방식으로 별도 해결했습니다.
